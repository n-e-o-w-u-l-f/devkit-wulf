...
# Make the offline fixture independent of the CI host's libc implementation.
...
grep -Fx -- '-y' "$ARGS_LOG" >/dev/null
grep -Fx -- '--profile' "$ARGS_LOG" >/dev/null
grep -Fx -- 'minimal' "$ARGS_LOG" >/dev/null
grep -Fx -- '--default-toolchain' "$ARGS_LOG" >/dev/null
grep -Fx -- 'stable' "$ARGS_LOG" >/dev/null
grep -Fx -- '--no-modify-path' "$ARGS_LOG" >/dev/null
...
# Existing exact managed installation is observed without any network access.
: > "$DOWNLOAD_LOG"
install_rustup_artifact linux amd64
[ ! -s "$DOWNLOAD_LOG" ]
[ "$(tail -n 1 "$STATE_DIR/rustup-artifact.jsonl" | jq -r '.action')" = observed-managed ]

# Tampering with a managed binary invalidates the marker-bound verification.
printf '\n# tampered\n' >> "$CARGO_HOME/bin/cargo"
if verify_rustup_artifact linux amd64; then echo "tampered cargo unexpectedly verified" >&2; exit 1; fi
rm -rf "$CARGO_HOME" "$RUSTUP_HOME"

# A foreign pre-existing home is refused before network access.
mkdir -p "$CARGO_HOME"
: > "$DOWNLOAD_LOG"
...
# Checksum mismatch hard-fails before rustup homes are created.
download_https() {
  url=$1 dest=$2
  printf '%s\n' "$url" >> "$DOWNLOAD_LOG"
  case "$url" in
...
# Linux artifact use is explicitly glibc-gated.
