...
$StateJunctionCreated = $false
$DevelopJunctionCreated = $false
...
try {
    ...
    Reset-DownloadLog
    Install-DevkitFlutterWindowsArtifact -ManifestPath $ManifestPath -HomePath $HomePath -StateDirectory $StateDir -Architecture 'amd64'
    $downloads = @(Get-Content -LiteralPath $DownloadLog | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    ...
    Remove-TestDirectory $Destination
    ...
    if (@(Get-Content -LiteralPath $DownloadLog | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }).Count -ne 0) { throw 'Foreign Flutter conflict was checked after network access.' }
    Remove-TestDirectory $Destination
    ...
    if (@(Get-Content -LiteralPath $DownloadLog | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }).Count -ne 0) { throw 'PATH gate ran after network access.' }
    $env:PATH = "$ManagedBin;$OldPath"
    ...
    $stream = [IO.File]::Open($UnsafeZip, [IO.FileMode]::Create, [IO.FileAccess]::ReadWrite, [IO.FileShare]::None)
    $zip = [IO.Compression.ZipArchive]::new($stream, [IO.Compression.ZipArchiveMode]::Create, $false)
    try {
        $entry = $zip.CreateEntry('../evil.txt')
        $writer = [IO.StreamWriter]::new($entry.Open())
        try { $writer.Write('evil') } finally { $writer.Dispose() }
        foreach ($name in @('flutter/bin/flutter.bat', 'flutter/bin/dart.bat')) {
            $entry = $zip.CreateEntry($name)
            $writer = [IO.StreamWriter]::new($entry.Open())
            try { $writer.Write('@echo off') } finally { $writer.Dispose() }
        }
    } finally {
        $zip.Dispose()
        $stream.Dispose()
    ...
    Remove-Item -LiteralPath $Develop -Force
    $DevelopJunctionCreated = $false
    ...
    $env:PATH = $OldPath
    if ($StateJunctionCreated -and (Test-Path -LiteralPath $StateDir)) { Remove-Item -LiteralPath $StateDir -Force -ErrorAction SilentlyContinue }
    if ($DevelopJunctionCreated -and (Test-Path -LiteralPath $Develop)) { Remove-Item -LiteralPath $Develop -Force -ErrorAction SilentlyContinue }
