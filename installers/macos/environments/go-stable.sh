...
case "$(uname -m 2>/dev/null || printf unknown)" in
    ...
    *) fail "No verified Go stable macOS artifact is mapped for this architecture." ;;
esac
...
download_https() {
    ...
    command -v curl >/dev/null 2>&1 || fail "curl is required to download verified Go artifacts on macOS."
...
case "$ACTION" in
    plan)
        ...
        plan_go_artifact macos "$ARCH" || fail "Unable to plan verified Go stable artifact."
        ;;
    verify)
        verify_go_artifact macos "$ARCH" || fail "Managed Go stable artifact verification failed."
        printf '%s\n' 'result=verified'
        ;;
    install)
        install_go_artifact macos "$ARCH"
        verify_go_artifact macos "$ARCH" || fail "Go installation completed but managed verification failed."
