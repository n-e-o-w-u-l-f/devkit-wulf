...
[ "$is_wsl" -eq 1 ] || fail "WSL was not detected. Use the native Linux Go adapter on native Linux."

SCRIPT_DIR=$(CDPATH= cd -P "$(dirname "$0")" && pwd)
LINUX_ADAPTER=$(CDPATH= cd -P "$SCRIPT_DIR/../../linux/environments" && pwd)/go-stable.sh
[ -x "$LINUX_ADAPTER" ] || fail "Shared Linux Go stable adapter is missing or not executable: $LINUX_ADAPTER"
