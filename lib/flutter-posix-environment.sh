...
# Caller provides: die, sha256_file, install_verified_artifact, plan_verified_artifact,
...
flutter_stable_marker_valid() {
  ...
  jq -e '
    ...
    and (.source_url | type == "string" and startswith("https://storage.googleapis.com/flutter_infra_release/releases/"))
...
flutter_stable_write_critical_hashes() {
  ...
  [ -x "$_dw_fs_root/bin/flutter" ] || die "managed Flutter executable is missing"
  [ -x "$_dw_fs_root/bin/dart" ] || die "managed Dart executable is missing"
  ...
  [ ! -e "$_dw_fs_tmp" ] || die "refusing existing Flutter marker staging file"
...
verify_flutter_stable_managed() {
  ...
  flutter_stable_marker_valid "$_dw_fs_root" || return 1
  _dw_fs_marker="$_dw_fs_root/.devkit-wulf-artifact.json"
...
install_flutter_stable_managed() {
  ...
  if [ -e "$_dw_fs_root" ] || [ -L "$_dw_fs_root" ]; then
    verify_flutter_stable_managed && return 0
  ...
  verify_flutter_stable_managed || die "GATE-12 Flutter stable managed verification failed after installation"
}
